...
await form.addEventListener('submit', async function (event) {
    ...
    const decadeNull = document.getElementById('decade_null').checked;
...
queryJson.onclick = async () => {
    ...
    const response = await fetch("http://localhost:8000/query-json?" + (year === "" ? "" : "year=" + year) + "&x=" + x + "&y=" + y + "&x2=" + x2 + "&y2=" + y2, {
        method: "GET",
        headers: {"Content-Type": "application/json"},
    });
    resp3.innerText = await response.json();
};